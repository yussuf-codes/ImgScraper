...
def ScrapeImages(search_query: str, images_number: int, download_location: str) -> None:
    THUMBNAIL_CLASS_NAME = "Q4LuWd"
    BUTTON_CLASS_NAME = "FAGjZe"
    IMAGE_CLASS_NAME = "iPVvYb"
    RESULTS_END_CLASS_NAME = "OuJzKb"
    ...
    database = f"{download_location}/images"

    if not path.exists(database):
        mkdir(database)

    directory = f"{database}/{search_query}"
